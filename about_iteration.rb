...
class AboutIteration < Neo::Koan
  ...
  def test_inject_will_blow_your_mind
    ...
    #   inject runs a block on each element passed in: `item` in code above
    #   it holds a stored accumulator that may be operated on: `sum` and `product` above
    #     - accumulator's starting value is passed in the inject(),
    #     - so the first one starts with a sum of 0, the second with a sum of 1
    #   the accumulator is manipulated and passed between each next iteration of the block
    #   ultimately the manipulated accumulator is returned
  end
